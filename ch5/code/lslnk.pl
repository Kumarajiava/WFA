...
print "Access Time       = ".gmtime($atime)." (UTC)\n";
print "Creation Date     = ".gmtime($ctime)." (UTC)\n";
print "Modification Time = ".gmtime($mtime)." (UTC)\n";
print "\n";
...
sub parseHeader {
	...
	$hdr{mtime}    = getTime($hd[5],$hd[6]);
	$hdr{atime}    = getTime($hd[7],$hd[8]);
	$hdr{length}   = $hd[9];
	$hdr{icon_num} = $hd[10];
	$hdr{showwnd}  = $hd[11];
	$hdr{hotkey}   = $hd[12];
