#! c:\perl\bin\perl.exe
#--------------------------------------------------------------------
# proc.pl - Perl script to retrieve process information
...
foreach (keys %processes) {
	print "PID       : ".$_."\n";
	print "Name      : ".$processes{$_}{'Name'}."\n";
	print "User      : ".$processes{$_}{'User'}."\n";
	my $parentname = $processes{$processes{$_}{'ParentPID'}}{'Name'};
	print "Parent PID: ".$processes{$_}{'ParentPID'}." [".$parentname."]\n";
	
	print "CmdLine   : ".$processes{$_}{'CommandLine'}."\n"; 
	print "Exe       : ".$processes{$_}{'ExecutablePath'}."\n";
	my @svcs = ();
